...
const CONRACT_OWNER_ADDRESS = process.env.CONRACT_OWNER_ADDRESS;
...
async function main() {
  ...
  try {
    ...
    const mintTx = await deckNFT.mintBaseDeck(CONRACT_OWNER_ADDRESS, METADATA_URI);
    ...
    if (event) {
      ...
      console.log("Backend Wallet (Owner):", owner);
  ...
  try {
    console.log("\n================Cloning deck...================");
    const cloneTx = await deckNFT.cloneDeck(PLAYER_WALLET_ADDRESS, baseDeckId, CLONED_METADATA_URI);
    ...
    if (cloneEvent) {
      baseDeckId = cloneEvent.args[0];
      clonedDeckId = cloneEvent.args[1];
      const playerAddress = cloneEvent.args[2];

      console.log(`Base Deck ID: ${baseDeckId}`);
      console.log(`Cloned Deck ID: ${clonedDeckId}`);
  ...
  try {
    console.log(`\n================Updating metadata URI for deck ID ${baseDeckId}...================`);
    const updateTx = await deckNFT.updateBaseMetadata(baseDeckId, NEW_METADATA_URI);
    ...
    console.log(`Updated Metadata URI: ${updateEvent.args.newMetadataURI}`);
  } catch (error) {
